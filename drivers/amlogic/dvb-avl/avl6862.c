					...
					struct i2c_adapter *i2c)
{
	...
	memcpy(&priv->frontend.ops, config->only_dvb_tc ? &avl6762_ops : &avl6862_ops,
	...
	if (fid != 0x68624955) {
		dev_err(&priv->i2c->dev, "%s: attach failed family id(0x%08x) mismatch",
				KBUILD_MODNAME, fid);
		goto err1;
	}
	switch (id) {
	...
	if (!avl6862_set_dvbmode(&priv->frontend, config->only_dvb_tc ? SYS_DVBT : SYS_DVBS))
